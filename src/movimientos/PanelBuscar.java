...
public class PanelBuscar extends JPanel implements ActionListener {
	...
	public String[] tbColumnas = {"No. Empleado","Nombre","Rol","Tipo","Fecha","Cantidad Entregas","Cubrio Turno", "ID"};
	/**
	 * Create the panel.
	 */
	public PanelBuscar() {
			...
			JScrollPane pane = new JScrollPane(tbBusqueda,JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		public void llenarLista(){
			...
			try {
			    ...
			    ResultSet rs = statement.executeQuery("SELECT * FROM movimientosrinku");
		...
		public void darBaja() {
			Statement statement;
			sID=tbBusqueda.getValueAt(tbBusqueda.getSelectedRow(), 7).toString();
			try {
				statement = con.getConnection().createStatement();
				String sql="DELETE from movimientosrinku WHERE id ="+sID;
		...
		public void buscarEmpleado() {
			...
			try {
				statement = con.getConnection().createStatement();
			    ResultSet rs = statement.executeQuery("SELECT * FROM movimientosrinku WHERE numemp="+txtEmp.getText());
			    ResultSetMetaData metaDatos = rs.getMetaData();
			    while (rs.next())
			    {
			    	int numeroColumnas = metaDatos.getColumnCount();
